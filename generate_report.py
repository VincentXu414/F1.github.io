...
def generate_html(data: dict) -> str:
    ...
    <div class="quiz-banner" onclick="fqToggle()">
        ...
        <span class="qb-sub">两套题库 &#183; 点击展开挑战自己</span>
...
<script>
    ...
    let fqSet = 'A', fqQ = 0, fqScore = 0, fqAns = [];
    function fqToggle() {{
    ...
    function fqShow() {{
        const set = fqData[fqSet], q = set.questions[fqQ];
        document.getElementById('fqQNum').textContent = '第 ' + (fqQ + 1) + ' / ' + set.questions.length + ' 题 [' + set.name + ']';
        document.getElementById('fqQText').textContent = q.q;
        document.getElementById('fqProgFill').style.width = (fqQ / set.questions.length * 100) + '%';
    ...
    function fqSelect(idx, btn) {{
        ...
        document.getElementById('fqNextBtn').textContent = fqQ === set.questions.length - 1 ? '查看结果' : '下一题';
        document.getElementById('fqProgFill').style.width = ((fqQ + 1) / set.questions.length * 100) + '%';
    }}
    function fqNext() {{
        const set = fqData[fqSet];
        if (fqQ < set.questions.length - 1) {{ fqQ++; fqShow(); }}
        else {{ fqResult(); }}
    }}
    function fqResult() {{
        document.getElementById('fqMain').style.display = 'none';
        document.getElementById('fqResult').style.display = 'block';
        const total = fqScore * 10;
        document.getElementById('fqScore').textContent = total + ' / 100';
        let r;
        if (total === 100) r = '完美！你就是F1百科全书';
        else if (total >= 80) r = '资深车迷！知识面很广';
        else if (total >= 60) r = '不错的车迷，还有提升空间';
        else if (total >= 40) r = '继续关注F1，你会越来越懂';
